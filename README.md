...
The Datube uses a series of holepunching techniques to make sure connectivity works on most networks,
...
```sh
npm install -g datube
datube # runs a DHT node
datube --bootstrap # runs a DHT node with bootstrap settings
datube --nodes 5 # runs 5 nodes
```
