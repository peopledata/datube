...
async function connectAndHolepunch (c, opts) {
  ...
  if (!c.connect) { // TODO: just a quick fix for now, should retry prob
    c.encryptedSocket.destroy(HANDSHAKE_INVALID())
    return
  }
...
async function holepunch (c, opts) {
  ...
  if (!remoteHolepunchable) {
    c.encryptedSocket.destroy(CANNOT_HOLEPUNCH())
    return
  }
...
async function connectThroughNode (c, address, socket) {
  ...
  if (payload.version !== 1) {
    c.encryptedSocket.destroy(SERVER_INCOMPATIBLE())
    return
  }
  if (payload.error !== ERROR.NONE) {
    c.encryptedSocket.destroy(SERVER_ERROR())
    return
  }
  if (!payload.udx) {
    c.encryptedSocket.destroy(SERVER_ERROR('Server did not send UDX data'))
    return
  }
